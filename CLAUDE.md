...
- **Adding guests never re-seats anyone.** With "Replace & re-seat everyone" unticked, `applyImport` appends the new guests and stops — no `randomize()`, so existing tables are byte-for-byte unchanged and the new names land in Not seated (unless the sheet gave them a `Table #`). Only a tick in Replace clears the tables and reshuffles.
